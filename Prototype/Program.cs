...
namespace Prototype
{
    public class Program
    {
        static void Main(string[] args)
        {
            ...
            //2) BUT, in fact we already have weather forecast for august, so don't reuse that data ?

            var julyAndAugustWeatherForecastForQutaisi = augustWeatherForecastForQutaisi.Clone();
            //Now we only need to load July, since we already have august.
            augustWeatherForecastForQutaisi.Load(DateTime.Parse("2017/07/01"), DateTime.Parse("2017/07/31"));
